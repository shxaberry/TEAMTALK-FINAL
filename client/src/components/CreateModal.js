...
const CreateModal = ({ 
  ...
  // Helper to handle the async create action with loading
  ...
  const isStep1Valid = roomTitle.trim().length > 0;

  return (
    <div className="fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-md overflow-y-auto px-4 py-8">
          ...
          <div className="px-10 pb-10">
            ...
            {step === 1 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                <button
                  ...
                  className={`w-full font-extrabold py-5 rounded-2xl shadow-xl transition ${
                    isStep1Valid 
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                <div className="flex gap-4">
                  <button
                    disabled={isCreating}
                    onClick={() => setStep(1)}
                    className="flex-1 py-5 bg-gray-50 text-gray-500 font-extrabold rounded-2xl disabled:opacity-50"
                  >
                    Back
                  </button>
                  ...
                  <button
                    ...
                    disabled={isCreating}
                    className={`flex-[2] text-white font-extrabold py-5 rounded-2xl shadow-xl transition flex items-center justify-center gap-2 ${
                      isCreating ? "bg-brand-400 cursor-wait" : "bg-brand-500 shadow-brand-500/20 hover:bg-brand-600"
                    }`}
                  >
                    {isCreating ? (
                      <>
            ...
            {step === 3 && (
              <div className="text-center py-4 animate-in zoom-in duration-500">
                ...
                <button onClick={onClose} className="w-full bg-brand-500 text-white font-extrabold py-5 rounded-2xl shadow-xl shadow-brand-500/20 hover:bg-brand-600 transition">
                  Open Workspace
                </button>
              </div>
            )}
